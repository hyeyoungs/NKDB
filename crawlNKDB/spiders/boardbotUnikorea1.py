...
class Boardbotunikorea1Spider(scrapy.Spider):
    ...
    def parse(self, response):
        page_no = 1
        last_page_text = response.xpath('//*[@id="container"]/div/section/div[1]/div[2]/a[3]/@href').extract()
        ...
        while True:
            ...
            #print(link)
            last = response.xpath('//*[@id="sublist"]/ul[1]/li[1]/div/label/text()').get()
            first = response.xpath('//*[@id="sublist"]/ul[10]/li[1]/div/label/text()').get()
    ...
    def parse_each_pages(self, response):
        ...
        while True:
            ...
            yield scrapy.Request(url, callback=self.parse_post, meta={'item':item, 'title': title})
            category_no += 1
