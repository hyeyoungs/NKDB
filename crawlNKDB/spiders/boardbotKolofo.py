...
class BoardbotkolofoSpider(scrapy.Spider):
    name = 'boardbotKolofo'
    allowed_domains = ['http://www.kolofo.org']
    start_urls = ['http://www.kolofo.org/?c=user&mcd=sub03_02']
    ...
    def __init__(self):
        ...
        self.start_urls = 'http://www.kolofo.org/?c=user&mcd=sub03_02'
	...
	# 각 항목마다 bodys, titles, writers, dates를 가져온다.
        ...
        if file_icon:
            ...
            if file_icon.find("hwp") != -1 :
                print('find hwp')
                yield scrapy.Request(file_download_url, headers = self.headers, callback=self.save_file_hwp, meta={'item':item}) #
            else:
                yield scrapy.Request(file_download_url, headers = self.headers, callback=self.save_file, meta={'item':item})
        else:
