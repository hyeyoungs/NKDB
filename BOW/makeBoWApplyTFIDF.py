...
def make_doclist(collection):
    ...
    for doc in docs:
        if str(type(doc["post_title"])) != "<class 'str'>":
            continue
        temp_list = doc["post_title"]
        if str(type(doc["post_body"])) == "<class 'str'>":
...
def applyTfidf(BoW):
    ...
    for index in range(0, len(BoW)):
        total_vector_list.append(model[BoW[index]])
        print(total_vector_list[index])
    print(model)

    return total_vector_list
Tfidf_vector_list = applyTfidf(BoW)
...
#
#
# for index in range(1, len(Tfidf_vector_list)):
#     score_dict = {}
#     similarity = cosine_similarity(Tfidf_vector_list[0], Tfidf_vector_list[index])
#     score_dict[index] = similarity
#
# for key, value in sorted(score_dict.items(), reverse=True):
#     print(key, ":", value)
